...
def gen_weights(coord): 
    """
    Generate weights
    In this case, distance from one node to other
    """
    weights = []
    for source in coord: 
        each_source_weight = []
        for dest in coord:
            if source != dest: 
                temp_weight = calc_dist(source, dest)
                each_source_weight.append(int(temp_weight))
        weights.append(each_source_weight)
    print(weights)
